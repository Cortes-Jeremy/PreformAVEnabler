...
StaticPopupDialogs["PREFORM_LEAVE_CONFIRM"] = {
	...
	OnCancel = function (self, data)
		-- reversing the buttons so leave queue is button two because people are retarded (OnCancel == button2)
...
function PreformAVEnabler_GetBGInfo()

	NUM_BGS = GetNumBattlegroundTypes();

	for id = 1, NUM_BGS do

		BG_NAME[id] = GetBattlegroundInfo(id);

		if ( not BG_ABR_NAME[id] ) then
			BG_ABR_NAME[id] = string.sub(BG_NAME[id], 1, 3);
		end

		if ( not PreformAVEnabler_MyStatus[id] ) then
			PreformAVEnabler_MyStatus[id] = { status = 0, id = 0 };
		end
	end
...
function PreformAVEnabler_Queue(bg, queueId)
	...
	if ( queueId and queueId > 0 ) then
		return;				-- queue from anywhere no longer allows queuing for a specific instance
	end
	...
	for id = 1, NUM_BGS do
		...
		if ( GetBattlegroundInfo(id) == BG_NAME[bg] ) then
			...
			PreformAVEnabler_AnywhereQueueBG = id;
...
--[[
	...
	elseif ( event == "PVPQUEUE_ANYWHERE_SHOW" ) then
		...
		if ( instances ) then
			PreformAVEnablerNumInstancesFrameText:SetText(PREFORM_AV_ENABLER_NUM_INSTANCES..": "..GetNumBattlefields());
		else
			PreformAVEnablerNumInstancesFrameText:SetText("");
		end
...
function PreformAVEnablerJoinButton_OnClick(self)
	...
	if ( bg == BG_ID[RANDOM_BATTLEGROUND] ) then
		-- ignore tally for random bgs since they have no instance IDs
		SendAddonMessage("PreformAVEnabler", "enter,"..bg, "RAID");

	elseif ( majorityBG > 0 ) then
		if ( bg == BG_ID[PREFORM_AV_ENABLER_AV] ) then
...
function PreformAVEnablerHideButton_OnClick(button, id)
	...
	local frameWidth = columnWidth * NUM_BGS + 195;
	...
	if ( PreformAVEnabler_HideColumn[id] ) then
		...
		getglobal("PreformAVEnablerHeaderText"..id):SetWidth(columnWidth);
		getglobal("PreformAVEnablerMajorityText"..id):SetWidth(columnWidth);
...
--		getglobal("PreformAVEnablerMajorityText"..id):SetText("");
	...
	for i = 1, 40 do
		getglobal("PreformAVEnablerMember"..i):SetWidth(frameWidth - 45 - (x * columnNoButtonWidth));
	end
end
